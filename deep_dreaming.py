...
def render_deepdream(t_obj, img0, iter_n=10, step=1.5, octave_n=4, ocatve_scale=1.4):
    """
...
image = PIL.Image.open("mountain2.jpg")
image = np.float32(image)

# The objective to visualise
#objective = T(layer_i)[:, :, :, channel_i] + T(layer_i)[:, :, :, channel_ii] + T(layer_i)[:, :, :, channel_iii]
objective = tf.square(T(layer_ii))

# Render the image
render_deepdream(objective, img0=image, octave_n=4)
